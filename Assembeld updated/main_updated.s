...
main:
	...
	jne	.L2
	mov	rbx, rsp
	mov	rax, QWORD PTR -176[rbp]	# rax = argv[0]
...
.L8:
	...
	mov	r13d, 0	# r13d = i в for {fscanf()}
	jmp	.L9
.L10:	# цикл for {fscanf()}
	mov	eax, r13d	# eax = i
	cdqe
...
.L9:
	...
	mov	r12d, 0	# r12d = i в for {fscanf()}
	jmp	.L11
.L12:	# цикл for {fprintf()}
	mov	rax, QWORD PTR -128[rbp]
	mov	edx, r12d	# edx = r12d
	movsx	rdx, edx
	mov	edx, DWORD PTR [rax+rdx*4]
	mov	rax, QWORD PTR -144[rbp]	# rax копирует поток вывода в файл
	lea	rsi, .LC5[rip]
	mov	rdi, rax
	mov	eax, 0
	call	fprintf@PLT
...
.L11:
	...
	cmp	r12d, eax	# i < size
	jl	.L12
	mov	rdi, QWORD PTR -144[rbp]	# rdi копирует поток вывода в файл
	call	fclose@PLT
	mov	eax, 0
	mov	rsp, rbx
	jmp	.L30
.L2:	# if (argc != 2)
	...
	mov	eax, 0
	call	printf@PLT
	lea	rax, -148[rbp]	# кладем по ссылке в rax size
	mov	rsi, rax
	lea	rdi, .LC2[rip]
	mov	eax, 0
...
.L21:
	...
	mov	edi, 10
	call	putchar@PLT
	mov	r14d, 0		# r14d = i в for {printf("A[%d] = %d\r\n", i, *(A + i))}
	jmp	.L23
.L24:
	mov	eax, r14d	# eax = i
	mov	esi, eax
...
.L28:
	...
	lea	rdi, .LC14[rip]
	call	puts@PLT
	mov	edx, DWORD PTR -148[rbp]	# eax = size для передачи в create_b(A, B, size)
	mov	rcx, QWORD PTR -96[rbp]		# rcx = B для передачи в create_b(A, B, size)
	mov	rax, QWORD PTR -80[rbp]		# rax = A для передачи в create_b(A, B, size)
	mov	rsi, rcx
	mov	rdi, rax
	call	create_b@PLT
	mov	edx, DWORD PTR -148[rbp]	# size для передачи в create_b(A, B, size)
	mov	rax, QWORD PTR -96[rbp]		# B для передачи в create_b(A, B, size)
	mov	esi, edx
	mov	rdi, rax
	call	print_mass@PLT
	mov	eax, 0
